...
const { RPC_URL, OWNER_KEY, SITE_EMAIL } = require("../../../env");
var Web3 = require("web3");
var web3 = new Web3(new Web3.providers.HttpProvider(RPC_URL));
var cp = require("child_process");
var mailer = require("./../../common/controller/mailController");
const rootPath = __dirname.replace("module/item/controller", "media/");
const web3RootPath = __dirname.replace("module/item/controller", "web3/");
        ...
        function (err, receiver) {
          userController.getUserInfoByID(
            ...
            function (err, sender) {
              ...
              this.transferAdminComission(item, function (error, commission) {
                this.transferBalance(
                  ...
                  function (is_transferred) {
                    ...
                    var command = `sh ${web3RootPath}transaction.sh ${receiver_address.id} ${sender_address.id} ${item.token_id} ${item.collection_id.contract_address} ${rootPath}contracts/${symbolabi} ${OWNER_KEY} ${RPC_URL}`;
                          ...
                          function (err, collection) {
                            ...
                            collection.save(function (err, collectionsaveObj) {
                              ...
                              item.save(function (err, itemObj) {
                                ...
                                history.history_type = "admin_commission";
...
exports.list = function (req, res) {
  ...
  if (req.query.type == "mycollection" && req.decoded.user_id != null) {
    ...
    if (req.query.type == "my") {
      ...
      query = query
        .where("current_owner", req.query.user_id)
  ...
  userController.getUserInfoByID(req.decoded.user_id, function (err, user) {
    web3.eth.getBalance(user.public_key).then((balance) => {
      ...
      if (eth > req.body.amount) {
        var command = `sh ${web3RootPath}send.sh ${user.private_key} ${req.body.eth_address} ${req.body.amount} ${RPC_URL}`;
        cp.exec(command, function (err, stdout, stderr) {
          if (err) {
            res.json({
              status: false,
...
exports.generateHash = function (req, res) {
  ...
  var command = `sh ${web3RootPath}generate.sh ${symbol} ${req.body.name} ${symbolsol} ${RPC_URL}`;
  cp.exec(command, function (err, stdout, stderr) {
    if (err) {
      res.json({
        status: false,
