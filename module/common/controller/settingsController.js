...
exports.installOptions = function (req, res) {
  ...
  var user = new users();
  user.username = "admin";
  user.email = "admin@globalvoxinc.com";
  user.password = "Global4321#";
  ...
  user.role = 1;
  user.status = "active";
  const rootPath = __dirname.replace("module/common/controller", "web3/");
  var command = `sh ${rootPath}address.sh ${RPC_URL}`;
...
exports.getOptions = function (req, res) {
  options.findOne({ name: req.query.name }, function (err, option) {
    ...
    res.json({
      status: true,
      message: "Options detail retrieved successfully.",
      result: option,
    });
  });
};
